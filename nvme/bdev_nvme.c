...
{
	nbdev_ch->current_io_path = NULL; //ljx
...
any_io_path_may_become_available(struct nvme_bdev_channel *nbdev_ch)
{
	...
	// STAILQ_FOREACH(io_path, &nbdev_ch->io_path_list, stailq) {
	// 	printf("any_io_path_may_become_available:io_path addr=%p\n",io_path);
...
struct io_device {
	void				*io_device;
	char				name[256 + 1];	// SPDK_MAX_DEVICE_NAME_LEN=256
...
// };
// struct ump_channel_list g_ump_bdev_channels;
/*=========================================================================================*/
static void
_bdev_nvme_reset_complete(struct spdk_io_channel_iter *i, int status)
{
	...
	if (!success) {
		...
		SPDK_NOTICELOG("spdk_io_channel:%p\n", i->ch);	// tmp
		SPDK_NOTICELOG("io_device name:%s\n", i->dev->name);	// tmp Malloc1
		...
		// 	{
		// 		SPDK_NOTICELOG("iopath->bdev->name:%s, i->dev->name:%s\n",iopath->bdev->name, i->dev->name);
		// 		if (strstr(iopath->bdev->name, i->dev->name))
		// 		{
		// 			SPDK_NOTICELOG("iopath %s reconnect successful!\n",iopath->bdev->name);
