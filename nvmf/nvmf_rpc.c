				   ...
				   const struct spdk_json_val *params)
{
	...
	struct spdk_nvmf_tgt *tgt;
	int rc;
	printf("rpc_nvmf_subsystem_remove_listener\n");
	ctx = calloc(1, sizeof(*ctx));
	if (!ctx) {
		spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR, "Out of memory");
		return;
	...
	// 获取设备
	tgt = spdk_nvmf_get_tgt(ctx->tgt_name);
	printf("rpc_nvmf_subsystem_remove_listener→target name：%s\n", ctx->tgt_name);
	if (!tgt)
	{
		SPDK_ERRLOG("Unable to find a target object.\n");
		spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
		   ...
		   void *cb_arg, int status)
{
	...
	// sd
	spdk_uuid_generate(&ns_opts.uuid);
	memcpy(ns_opts.nguid, ns_opts.uuid.u.raw, sizeof(ns_opts.nguid));
	// sd
